...
<b>Dieses Dokument und der zugrundeliegende Code ist <i>Work-In-Progress</i> (seit 2015) und das Datum bezieht sich auf die letzte <i>wesentliche</i> Änderung im Code.</b>

<b>Der Datensatz wird laufend ergänzt.</b>

<b>Die Verwendete Version von R ist 4.4.3</b>
...
Folgende Daten werden anhand der Tankquittungen und Notizen/Photos auf einem 
Mobiltelefon (Kilometerstände) erfasst:
...
Tankstatistik.raw[1, 1] <- 20; Tankstatistik.raw[1, 2] <- 6

## Rekonstruktion von [1, 7] (Literpreis)
Tankstatistik.raw[1, 7] <- Tankstatistik.raw[1, 8]/Tankstatistik.raw[1, 6]
```
...
# in die Tabelle.
Datum <- paste(Tankstatistik.raw$Jahr,"-",Tankstatistik.raw$Monat,"-",Tankstatistik.raw$Tag, sep="")
Tankstatistik.raw$Datum <- as.Date(Datum)

# Berechnen der vergangenen Tage zwischen zwei Betankungen
Tage <- as.integer(diff(Tankstatistik.raw$Datum))
Tankstatistik.raw$Tage <- c(Tage, NA)
```
...
Tankstatistik.raw.2019 <- subset(Tankstatistik.raw, Jahr == "2019")
Tankstatistik.raw.2019 <- Tankstatistik.raw.2019[,c(9,12)]
Mittel.a.2019 <- lapply(Tankstatistik.raw.2019, function(x) mean(x))
Mittel.a.2019$km.Tag <- (Mittel.a.2019$km/Mittel.a.2019$Tage)
Tankstatistik.raw[58, 9] <- Tankstatistik.raw[58,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[59, 9] <- Tankstatistik.raw[59,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[60, 9] <- Tankstatistik.raw[60,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[61, 9] <- Tankstatistik.raw[61,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[62, 9] <- Tankstatistik.raw[62,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[63, 9] <- Tankstatistik.raw[63,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[64, 9] <- Tankstatistik.raw[64,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[65, 9] <- Tankstatistik.raw[65,12] * Mittel.a.2019$km.Tag
Tankstatistik.raw[66, 9] <- Tankstatistik.raw[66,12] * Mittel.a.2019$km.Tag
```
...
for (i in 76:2){
  Tankstatistik.raw[i-1, 10] <- Tankstatistik.raw[i, 10] - Tankstatistik.raw[i, 9]
}
```

#### Aufbereitung, Teil 2

Die Rekonstruktion ist an dieser Stelle abgeschlossen, weshalb der rekonstruierte Datensatz gespeichert wird. Des weiteren werden überflüssige Objekte gelöscht. <b>Hierzu gehört auch die jüngste Beobachtung, die noch keine zurückgelegten Kilometer enthält.</b>

```{r Aufbereitung Teil 2}
## Bereinigung
# Entfernen der, im Folgenden noch nicht verwedeten Uhrzeitvariablen
# sowie der unvollständigen letzten Beobachtung
n <- nrow(Tankstatistik.raw)
Tankstatistik.raw <- Tankstatistik.raw[-n ,-c(4,5)]

# Tag und Monat als Ganzzahlen speichern
Tankstatistik.raw$Tag <- as.integer(Tankstatistik.raw$Tag)
Tankstatistik.raw$Monat <- as.integer(Tankstatistik.raw$Monat)

# Weiterarbeit mit umbenannten rekonstruierten Datensatz
Tankstatistik <- Tankstatistik.raw

# Entfernen aller ab hier unnötigen Objekte
rm(Tankstatistik.raw, Tankstatistik.raw.2019, Mittel.a.2019, Datum, i, n, Tage)

## Bilden einiger Quotienten
# Liter je km
Tankstatistik$Liter.km <- Tankstatistik$Liter / Tankstatistik$km

# Euro je km
Tankstatistik$Euro.km <- Tankstatistik$Euro / Tankstatistik$km

# Kilometer je Tag
Tankstatistik$km.Tag <- Tankstatistik$km / Tankstatistik$Tage

# Liter je Tag
Tankstatistik$Liter.Tag <- Tankstatistik$Liter / Tankstatistik$Tage

# Euro je Tag
Tankstatistik$Euro.Tag <- Tankstatistik$Euro / Tankstatistik$Tage
```
...
der exportierten Statistik in eine SQL-Datenbank wird durch eine uneinheitliche 
Anzahl von Nachkommastellen sehr erschwert.
...
Tankstatistik$km <- round(Tankstatistik$km)
Tankstatistik$km_gesamt <- round(Tankstatistik$km_gesamt)
Tankstatistik$Liter.km <- round(Tankstatistik$Liter.km, 4)
Tankstatistik$Euro.km <- round(Tankstatistik$Euro.km, 4)
Tankstatistik$km.Tag <- round(Tankstatistik$km.Tag, 3)
Tankstatistik$Liter.Tag <- round(Tankstatistik$Liter.Tag, 4)
Tankstatistik$Euro.Tag <- round(Tankstatistik$Euro.Tag, 4)
```
...
Der Code zur Auswertung wird hier aufgrund großer Länge und Repetitivität ausgelassen und im zugrundliegenden Rmarkdown-Skript einsehbar.
...
Summen <- unlist(lapply(Tankstatistik[c(4, 6, 7, 10)], function(x) sum(x)))
# Anzahl der Betankungen
Summen <- c(Summen, dim(Tankstatistik)[1])
...
Mittel.a <- lapply(Tankstatistik[c(4:7, 10)], function(x) mean(x))
...
# Durchschnittlich gefahrene Strecke in Kilometern
# Durchschnittlich vergangenene Zeit zwischen zwei Betankungen in Tagen
Mittel.m <- lapply(Tankstatistik[c(4:7, 10)], function(x) median(x))
...
# Aufteilen nach Jahr
Tankstatistik.Jahr <- split(Tankstatistik, Tankstatistik$Jahr)


## Summen
# Gesamtverbrauch an Benzin in Litern
Gesamtverbrauch.Jahr <- unlist(lapply(Tankstatistik.Jahr, function(x) sum(x[, 4])))
# Gesamtausgaben in Euro
Gesamtausgaben.Jahr <- unlist(lapply(Tankstatistik.Jahr, function(x) sum(x[, 6])))
# Insgesamt gefahrene Kilomenter
Kilometer.Jahr <- unlist(lapply(Tankstatistik.Jahr, function(x) sum(x[, 7])))
# Anzahl der Betankungen
Betankungen.Jahr <- unlist(lapply(Tankstatistik.Jahr, function(x) dim(x)[1]))
...
Liter.Tank.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) mean(x[, 4])))
# Durschschnittspreis in Euro/Liter je Jahr
Literpreis.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) mean(x[, 5])))
# Durchschnittliche Ausgaben in Euro je Jahr
Euro.Tank.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) mean(x[, 6])))
# Durchschnittlich gefahrene Strecke in Kilometern je Jahr
KM.Tank.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) mean(x[, 7])))
# Durchschnittlich vergangenene Zeit zwischen zwei Betankungen in Tagen
Tage.Tank.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) mean(x[, 10])))

# Durchschnittsverbrauch in Liter/km
Liter.KM.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) (mean(x[, 4])/mean(x[, 7]))))
# Durchschnittskosten in Euro/km
Euro.KM.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) (mean(x[, 6])/mean(x[, 7]))))
# Durchschnittliche Kilometer am Tag
KM.Tag.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) (mean(x[, 6])/mean(x[, 10]))))
# Durchschnittlicher Benzinverbrauch in l/Tag
Liter.Tag.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) (mean(x[, 4])/mean(x[, 10]))))
# Durchschnittliche Kosten in Euro/Tag
Euro.Tag.Jahr.a <- unlist(lapply(Tankstatistik.Jahr, function(x) (mean(x[, 6])/mean(x[, 10]))))
...
## Mittel und Quotienten (median)
# Durchschnittlich getanktes Benzin in Litern je Jahr
Liter.Tank.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) median(x[, 4])))
# Durschschnittspreis in Euro/Liter je Jahr
Literpreis.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) median(x[, 5])))
# Durchschnittliche Ausgaben in Euro je Jahr
Euro.Tank.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) median(x[, 6])))
# Durchschnittlich gefahrene Strecke in Kilometern je Jahr
KM.Tank.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) median(x[, 7])))
# Durchschnittlich vergangenene Zeit zwischen zwei Betankungen in Tagen
Tage.Tank.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) median(x[, 10])))

# Durchschnittsverbrauch in Liter/km
Liter.KM.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) (median(x[, 4])/median(x[, 7]))))
# Durchschnittskosten in Euro/km
Euro.KM.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) (median(x[, 6])/median(x[, 7]))))
# Durchschnittliche Kilometer am Tag
KM.Tag.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) (median(x[, 6])/median(x[, 10]))))
# Durchschnittlicher Benzinverbrauch in l/Tag
Liter.Tag.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) (median(x[, 4])/median(x[, 10]))))
# Durchschnittliche Kosten in Euro/Tag
Euro.Tag.Jahr.m <- unlist(lapply(Tankstatistik.Jahr, function(x) (median(x[, 6])/median(x[, 10]))))
...
Mittel.diff.Jahr <- Mittel.Jahr.m - Mittel.Jahr.a
...
knitr::kable(Mittel.diff.Jahr, caption = "Absolute Differenz Median - Arith. Mittel", digits = 2,
             col.names = c("Liter", "Euro/Liter", "Euro", "km", "Tage", "Liter/km",
                         "Euro/km", "km/Tag", "Liter/Tag", "Euro/Tag"))
```
...
# Tankstatistik als Tibble
Tankstatistik <- as_tibble(Tankstatistik)
...
Abb.Liter <- ggplot(data = Tankstatistik,
...
Abb.Literpreise <- ggplot(data = Tankstatistik,
...
Abb.Kilometer <- ggplot(data = Tankstatistik,
  ...
  scale_y_continuous(expand = c(0, 0),
                     limits = c(0, 1000)) +
  labs(x = "Jahr",
       y = "Kilometer/Betankung",
       title = "Gefahrene Kilometer je Betankung") +
  geom_line() +
  geom_point(size = 1) +
  annotate(geom = "text", x = as.Date("2016-04-16"), y = 900,
...
Abb.Kilometer.Liter <- ggplot(data = Tankstatistik,
...
Abb.Liter.100km <- ggplot(data = Tankstatistik,
...
Abb.Liter.Jahr <- ggplot(data = Mittel.Jahr) +
  ...
  geom_hline(yintercept = 40, color = "red") +
  annotate(geom = "text", x = 2015, y = 42, label = "max. Fassungsvermögen", hjust = "center")
Abb.Liter.Jahr
```
...
### Quelltexte und Verweise

#### R-Projekt online

Das vollständige Projekt ist Interessierte mit Benutzerkonto bei RStudio.Cloud unter <https://posit.cloud/content/3318758> einseh- und ausführbar.
